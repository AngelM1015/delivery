...
import { Modal, StyleSheet, View, Text, TouchableOpacity, Image, ScrollView, FlatList } from "react-native";
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 15 }}>
            ...
            <View style={{ marginLeft: 15, gap: 10 }}>
              <Text style={{ color: 'black', fontSize: 20, fontWeight: 'bold' }}>{order.restaurant_name}</Text>
          ...
          <Text style={{ color: 'black', fontWeight: '400', fontSize: 12 }}>{order.order_items.length} item</Text>
        </View>
      </View>
    );
  }
    ...
    >
      <View style={styles.modalContainer}>
        ...
        <ScrollView contentContainerStyle={styles.orderListContainer}>
          <FlatList data={partnerOrders} renderItem={renderOrderItem} keyExtractor={item => item.id.toString()}
