...
const FavoriteFoodMenuItemScreen = ({ route, navigation }) => {
  ...
  const fetchMenuItems = async () => {
    ...
    try {
      ...
      const menuResponse = await axios.get(
        `${base_url}api/v1/restaurants/${restaurantId}/menu_items/`,
        { headers },
      );
      ...
      if (userId) {
        try {
          const favResponse = await axios.get(
            `${base_url}api/v1/users/${userId}/favorites`,
            { headers },
          );
          const restaurantFavs = favResponse.data.filter(
            (item) => item.restaurant_id === parseInt(restaurantId),
          );
          updateState({
            favoriteItems: restaurantFavs.map((fav) => fav.menu_item),
          });
        } catch (err) {
  ...
  const isItemFavorite = useCallback(
    (item) => favoriteItems.some((fav) => fav.id === item.id),
    [favoriteItems],
  );
  ...
  const getFilteredItems = useCallback(() => {
    ...
    switch (sortOrder) {
      ...
      case "price-low":
        return [...items].sort(
          (a, b) => (a.item_prices?.[0] || 0) - (b.item_prices?.[0] || 0),
        );
      case "price-high":
        return [...items].sort(
          (a, b) => (b.item_prices?.[0] || 0) - (a.item_prices?.[0] || 0),
        );
      default:
        return items;
    }
  ...
  const toggleFavorite = async (item) => {
    if (!userId) {
      updateState({
        snackbarMessage: "You need to be logged in to add favorites.",
        snackbarVisible: true,
      });
      return;
    }
    ...
    try {
      ...
      if (isCurrentlyFavorite) {
        ...
        if (favItem) {
          await axios.delete(
            `${base_url}api/v1/users/${userId}/favorites/${favItem.id}`,
            { headers },
          );
          updateState({
      ...
      updateState({
        snackbarMessage: "Failed to update favorites. Please try again.",
        snackbarVisible: true,
      });
    }
  };
    ...
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyText}>
        {searchQuery
          ...
          : filterCategory
          ? `No items in category: ${filterCategory}`
          : showFavoritesOnly
  ...
  return (
    <>
      ...
      <Appbar.Header>
        ...
        <Menu
          visible={menuVisible}
          onDismiss={() => updateState({ menuVisible: false })}
          anchor={<View />}
          style={styles.menu}
        >
          <Menu.Item
            ...
            onPress={() =>
              updateState({ sortOrder: "name", menuVisible: false })
            }
          />
          <Menu.Item
            ...
            onPress={() =>
              updateState({ sortOrder: "price-low", menuVisible: false })
            }
          />
          <Menu.Item
            ...
            onPress={() =>
              updateState({ sortOrder: "price-high", menuVisible: false })
            }
          />
          <Divider />
          <Menu.Item
            ...
            icon="filter-remove"
            onPress={() =>
              updateState({ filterCategory: null, menuVisible: false })
            }
          />
        </Menu>
      </Appbar.Header>
      ...
      <Banner
        ...
        actions={[
          {
            label: "Dismiss",
            onPress: () => updateState({ bannerVisible: false }),
          },
        ]}
      ...
      <View style={styles.container}>
        <Searchbar
          ...
          onChangeText={(query) => updateState({ searchQuery: query })}
          value={searchQuery}
        ...
        <View style={styles.filterContainer}>
          ...
          >
            <Chip
              ...
              onPress={() => updateState({ filterCategory: null })}
          ...
          <View style={styles.viewToggle}>
            <ToggleButton.Row
              onValueChange={(value) =>
                value && updateState({ viewMode: value })
              }
              value={viewMode}
            >
      ...
      <Portal>
        <Dialog
          ...
          onDismiss={() => updateState({ dialogVisible: false })}
        >
          ...
          <Dialog.Content>
            {selectedItem && (
              <>
                ...
                <RadioButton.Group
                  onValueChange={(value) =>
                    updateState({ dialogOption: value })
                  }
                  value={dialogOption}
                >
                  <View style={styles.radioOption}>
                    <RadioButton value="view" />
                    <Text>View Details</Text>
                  </View>
                  {userRole === "customer" && (
                    <>
                      <View style={styles.radioOption}>
                        <RadioButton value="cart" />
                        <Text>Add to Cart</Text>
                      </View>
                      <View style={styles.radioOption}>
                        <RadioButton value="favorite" />
                        <Text>
                          {isItemFavorite(selectedItem)
                            ? "Remove from Favorites"
                            : "Add to Favorites"}
                        </Text>
                      </View>
          ...
          <Dialog.Actions>
            <Button onPress={() => updateState({ dialogVisible: false })}>
              Cancel
            </Button>
            <Button onPress={handleDialogConfirm} disabled={!dialogOption}>
              Confirm
            </Button>
          </Dialog.Actions>
        </Dialog>
